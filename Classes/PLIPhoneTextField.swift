...
@IBDesignable
class PLIPhoneTextField: UIView {
...
//            9: twitter // A type optimized for twitter text entry (easy access to @ #)
    ...
    @objc internal func textDidBegin(notificaiton: NSNotification) {
        self.horizontalLine.backgroundColor = AppColors.appOrange
    }
    
    @objc internal func textDidEnd(notificaiton: NSNotification) {
        self.horizontalLine.backgroundColor = AppColors.appGray
    }
    
    @objc internal func textDidChange(notificaiton: NSNotification) {
    ...
    fileprivate func createPhoneCodeBtn() {
        ...
        if UIDevice.deviceType() == .pad {
            ...
            phoneCodeBtn.titleLabel?.font = UIFont.systemFont(ofSize: 18.0)
        }
...
extension PLIPhoneTextField: UITextFieldDelegate {
